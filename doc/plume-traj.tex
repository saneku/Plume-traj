...
The backward-in-time simulation begins at a specified time (parameter \path{--start-time}) by defining the starting locations of the backward-in-time trajectories. The start time must fall within the WRF output time range. This process is called parcel seeding:
\begin{itemize}
...
\begin{itemize}
  \item \path{--source-lat}, \path{--source-lon} for a single column, or \path{--seed-bbox} with \path{--n-columns} for random source columns (available in both WRF and MPAS modes when \path{--emission-matrix} is not used).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \path{--hourly-output-dir}: If set, saves hourly parcel-location snapshots during the backward integration (e.g., \texttt{parcel\_positions\_hour\_XXX.png}) into the provided directory.
    \end{itemize}    
\end{itemize}
...
The script \texttt{plume\_forwtraj.py} is for forward runs and produces height- and age-colored trajectory maps (\path{--initial-height-figure}, \path{--age-figure}), as shown in Fig. \ref{fig:fig3}.
It can also save hourly parcel-location snapshots (\path{--hourly-output-dir}) and a deposited-parcel map colored by deposition hour (\path{--deposition-figure}).
For both WRF and MPAS forward runs, map diagnostics use consistent overlays: the source marker (red triangle, from \path{--source-lat}/\path{--source-lon}) and the seed area box (\path{--seed-bbox}, when provided). The same overlay behavior is preserved when re-plotting from \path{--state-pickle} with \texttt{plot\_forwtraj.py}.
