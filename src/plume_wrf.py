"""WRF backend for Plume-Traj.

This module contains the WRF reader, advection, plotting, CLI argument parsers, and backend facade.
"""
...
def _normalize_map_extent(map_extent):
    ...
    return normalize_map_extent(map_extent)
...
# Forward WRF implementation
FORWARD_TRAJECTORY_ALPHA = 1.0
...
def _combine_wrf_outputs_forw(wrfout_paths):
    grids = []
    for path in wrfout_paths:
        grids.append(read_wrf_geometry_and_winds(path))
...
def _expand_wrfout_paths_forw(input_args):
    expanded = []
    for entry in input_args:
        if any(ch in entry for ch in ["*", "?", "["]):
            matches = sorted(glob(entry))
...
def _plot_trajectory_map_base(xlat, xlon, map_extent=None, water_only_grid=True):
    lon_min, lon_max = float(np.nanmin(xlon)), float(np.nanmax(xlon))
    lat_min, lat_max = float(np.nanmin(xlat)), float(np.nanmax(xlat))
    lon_pad = max((lon_max - lon_min) * 0.05, 0.1)
    lat_pad = max((lat_max - lat_min) * 0.05, 0.1)
    fig, ax = _init_geo_axes(
        ...
        lat_pad,
        map_extent=map_extent,
        water_only_grid=water_only_grid,
    )
    return fig, ax
...
def plot_trajectories_by_height(
    ...
    """Plot trajectories colored by initial parcel height."""
    ...
    fig, ax = _plot_trajectory_map_base(
        xlat,
        xlon,
        map_extent=map_extent,
...
def parse_forwtraj_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional time-intensity emission TXT. Format: header "
            "'time_offset_h parcels', followed by rows of release time offset "
            "and total parcel count. Heights are generated uniformly from "
            "--z-min/--z-max/--n-vert."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="parcel_initial_heights.png",
...
def run_forwtraj(args):
    wrfout_paths = _expand_wrfout_paths_forw(args.input)
    ...
    grid = _combine_wrf_outputs_forw(wrfout_paths)
    xlat = grid["xlat"]
    xlon = grid["xlon"]
    u = grid["u"]
    v = grid["v"]
    ...
    if args.emission_matrix and args.emission_timeseries:
        raise ValueError("Use only one of --emission-matrix or --emission-timeseries.")

    has_emission_schedule = bool(args.emission_matrix or args.emission_timeseries)
    if np.isfinite(z_max_cfg) and z_max_cfg > model_max_height and not has_emission_schedule:
        raise ValueError(
    ...
    release_time_sec = None
    if args.emission_matrix or args.emission_timeseries:
    ...
    if saved_height:
        print(f"[diag] Initial-height figure saved to '{args.initial_height_figure}'.")
...
class WRFBackend(PlumeBackend):
    ...
    def plot_backtraj_state(self, args):
        from . import plot_backtraj_wrf

        plot_backtraj_wrf.main(args)

    def plot_forwtraj_state(self, args):
        from . import plot_forwtraj_wrf

        plot_forwtraj_wrf.main(args)
