...
python ./regrid_to_wrf.py \
  ...
  --dest-file-grid "/scratch/ukhova/SandBox/WRF/run_hayligubbi/wrfout_d01_2025-11-23_06:00:00" \
  --output-file '/lustre2/project/k10022/ukhova/Volcano/Hayli_Gubbi/operRSmerged_SO2_/AOD_AI_HEIGHT/100km/merged_aerosol_index_354_388_2025-NOV-24.nc' \
    ...
    with Dataset(dest_grid_path, "r") as dest_ds:
        if "XLAT" in dest_ds.variables and "XLONG" in dest_ds.variables:
            lat_var, lon_var = dest_ds.variables["XLAT"], dest_ds.variables["XLONG"]
            _diag("Found destination coordinate variables: XLAT, XLONG.")
            ...
            _diag("Could not find coordinate variables (XLAT/XLONG or lat/lon) in destination file.")
            sys.exit(1)
        dest_lat = lat_var[0, :, :] if lat_var.ndim == 3 else lat_var[:, :]
        dest_lon = lon_var[0, :, :] if lon_var.ndim == 3 else lon_var[:, :]
        _diag(f"Destination grid has shape {dest_lat.shape}.")
    ...
    with Dataset(dest_grid_path, "r") as dest_ds, Dataset(
        ...
        essential_vars = ["XLAT", "XLONG", "lat", "lon", "Times"]
        ...
        for var_name, src_values in source_vars.items():
            ...
            # Assuming variables are 3D (time, south_north, west_east)
            # Check if the variable exists in the destination file to get its dimensions
            if var_name in dest_ds.variables and 'Time' in dest_ds.variables[var_name].dimensions:
                out_var = out_ds.createVariable(
                    var_name, "f4", ("Time", "south_north", "west_east")
                )
                out_var[0, :, :] = regridded_data
            else: # Assuming 2D
                out_var = out_ds.createVariable(
                    var_name, "f4", ("south_north", "west_east")
                )
